    ...
    <div
      className={cn(
        isMobile
          ...
          // GSI fork: bump 154 → 220 — bottom action menu jest wyższy niż
          // upstream estimate (toolbar narzędzi + drugi rząd skrótów +
          // padding ~140-160px). Panel bottom = top-20 + (vh - 220) =
          // vh - 140, zostawia 140px na bottom bar bez clip'a. Tall panele
          // (Wymiary, Materiał, Hardware) wcześniej wystawały poza
          // viewport mimo overflow-y-auto, bo `flex-1` rozciągał content
          // do max-h cap który był za niski względem realnego bottom bar.
          : 'pointer-events-auto fixed top-20 right-4 z-50 flex max-h-[calc(100dvh-220px)] flex-col overflow-hidden rounded-xl border border-border/50 bg-sidebar/95 shadow-2xl backdrop-blur-xl dark:text-foreground',
        className,
      )}
